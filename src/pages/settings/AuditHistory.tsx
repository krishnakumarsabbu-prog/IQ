...
import { History, Calendar, User, Info, ArrowUpRight, Search, Eye } from 'lucide-react';
  ...
  return (
    <Layout>
      <div className="min-h-screen bg-slate-900/30 text-gray-100 p-6">
        ...
        {/* Audit Timeline */}
        <div className="bg-slate-800/60 border border-slate-750 p-6 rounded-2xl max-w-4xl mx-auto">
          <span className="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-6">Activity Logs Timeline</span>
            ...
            <div className="relative border-l border-slate-700 ml-4 space-y-6">
              {auditLogs.map((log) => (
                <div key={log.id} className="relative pl-8">
                  ...
                  <div className="bg-slate-900/40 border border-slate-750 rounded-xl p-4 space-y-3 hover:border-slate-700 transition-all">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        <span className={`text-xxs font-bold px-2 py-0.5 rounded ${getActionStyles(log.action)}`}>
                          {log.action}
                        </span>
                        <h4 className="text-sm font-bold text-gray-200">{log.templateName}</h4>
                      </div>
                    ...
                    <div className="flex justify-between items-center text-xxs text-gray-500 pt-2 border-t border-slate-800">
