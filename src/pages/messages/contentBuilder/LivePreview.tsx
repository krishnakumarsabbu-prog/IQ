...
function EmailBlock({
  ...
  switch (comp.kind) {
    ...
    case 'banner':
      return (
        <div className="py-6 px-5 font-bold text-white text-center text-sm leading-snug
          tracking-tight"
    ...
    case 'subheading':
      return (
        <h2 className="text-[13px] font-bold text-slate-800 px-5 pt-4 pb-1">{text}</h2>
      );

    case 'paragraph':
    case 'richtext':
      return (
        <p className="text-[12px] text-slate-600 px-5 py-2 leading-relaxed"
          dangerouslySetInnerHTML={{ __html: text }} />
      );

    case 'quote':
      return (
        <blockquote className="mx-5 my-3 pl-4 border-l-[3px] text-[12px] italic
      ...
      return (
        <div className="flex justify-center px-5 py-4">
          <a href={url} target="_blank" rel="noreferrer"
            className="inline-block px-7 py-2.5 text-[12px] font-bold text-white
              rounded-lg shadow-lg transition-opacity hover:opacity-90"
    ...
    case 'image':
      return (
        <div className="px-5 py-2">
          <div className="w-full h-20 rounded-lg bg-slate-100 border border-slate-200
    ...
    case 'accountdetails':
      return (
        <div className="mx-5 my-2 p-3.5 rounded-xl bg-slate-50 border border-slate-200
          text-[11px] space-y-1 text-slate-600">
          <p className="font-bold text-slate-700 text-xs">{comp.label}</p>
          <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 pt-0.5">
            <span className="text-slate-400">Account</span><span>{vars.AccountNumber ?? '****XXXX'}</span>
            <span className="text-slate-400">Amount</span><span className="font-semibold">{vars.Amount ?? '$0.00'}</span>
            <span className="text-slate-400">Date</span><span>{vars.TransactionDate ?? '—'}</span>
          </div>
        </div>
      );
    ...
    default:
      return (
        <div className="mx-5 my-1.5 px-3 py-2 rounded-lg border border-dashed
          border-slate-200 text-[11px] text-slate-400 italic">
          [{comp.label}: {text}]
        </div>
      );
  }
...
export default function LivePreview() {
  const {
    components, brand, previewSegment, setPreviewSegment,
    sampleDataKey, setSampleDataKey, currentSampleData,
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-5
        ...
        <div className="bg-white rounded-2xl shadow-xl shadow-slate-200/60
          ...
          {sorted.length === 0 ? (
            ...
            <div className="pb-1">
              {sorted.map(comp => (
                <EmailBlock key={comp.id} comp={comp}
                  primaryColor={brand.primaryColor} vars={vars} />
              ))}
            </div>
          )}
          ...
          <div className="px-5 py-4 border-t border-slate-100 dark:border-slate-800
            ...
            <p className="text-[10px] text-slate-400 leading-relaxed">
              © 2025 Wells Fargo Bank, N.A. All rights reserved.<br />
